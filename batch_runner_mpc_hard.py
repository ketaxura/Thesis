"""
batch_runner_mpc_hard.py
Runs MPC with HARD dynamic obstacle constraints (no slack).
Drop into MPC project folder alongside run_experiment_mpc_hard.py.

Usage: python batch_runner_mpc_hard.py
Output:
...
import os
import numpy as np
from run_experiment_mpc_hard import run_mpc_hard
...
for scenario in SCENARIOS:
    ...
    print(f"\n{'='*60}")
    print(f"  MPC (HARD)  Scenario: {label} (path_id={path_id})")
    print(f"{'='*60}")

    for seed in range(N_SEEDS):
        print(f"  seed {seed:2d} / {N_SEEDS-1} ...", end=" ", flush=True)
        try:
            result = run_mpc_hard(path_id=path_id, seed_offset=seed,
                                  max_steps=MAX_STEPS)
...
# Raw CSV
raw_path = "results/mpc_hard_results.csv"
...
summary_rows = []
print(f"\n{'='*60}")
print("  MPC (HARD) SUMMARY (mean ± std across seeds)")
print(f"{'='*60}")
...
sum_path = "results/mpc_hard_summary.csv"
if summary_rows:
