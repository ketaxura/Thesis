"""
run_experiment_mpc_v2.py
MPC experiment runner using the EXACT same environment as MPCC.

Instructions:
1. Copy obs.py from your MPCC project into your MPC project folder
2. Drop this file into your MPC project folder
3. Run batch_runner_mpc_v2.py

The only difference from MPCC is the controller formulation:
- MPC: quadratic tracking cost, time-indexed waypoints
- MPCC: contouring/lag/progress cost, geometric path following
"""
...
# Import the EXACT same obstacle environment as MPCC
# (copy obs.py from MPCC project into this folder)
...
# Override N to match MPCC horizon — MPC params.py has N=20 but MPCC uses N=30
# A fair comparison requires the same prediction horizon
N = 30

# Slew limits — match MPCC params
dv_max = 0.25
dw_max = 0.6
...
def build_mpc_solver(static_rects, dyn_obs, use_hard=False):
    """
    Build MPC NLP.
    use_hard=False : soft dynamic obstacle constraints (slack variables)
    use_hard=True  : hard dynamic obstacle constraints (no slack)
    """
    ...
    if not use_hard:
        for _ in range(len(dyn_obs) * N):
            lbx.append(0.0);  ubx.append(ca.inf)
...
def run_mpc(path_id: int, seed_offset: int,
            max_steps: int = 800, use_hard: bool = False) -> dict:
    """
    Run one MPC simulation on the identical environment as MPCC.
    use_hard=False: soft dynamic obstacle constraints
    use_hard=True : hard dynamic obstacle constraints
    """
    # Identical environment setup as MPCC
    ...
    # Initial state — identical to MPCC
    ...
    for k in range(max_steps):
        ...
        # Time-indexed reference window (key MPC difference from MPCC)
        ...
        # Horizon predictions — identical to MPCC
        if dyn_obs:
        ...
        # Step obstacles — identical timing to MPCC
        for obs in dyn_obs:
            obs.step(dt, STATIC_RECTS)

        # Collision detection — identical to MPCC
        ...
        # Geometric errors — identical to MPCC
    ...
    # Pack metrics — identical keys to MPCC
