"""
batch_runner_mpc.py
Runs MPC with SOFT dynamic obstacle constraints (slack variables).
Drop into MPC project folder alongside run_experiment_mpc.py.

Usage: python batch_runner_mpc.py
Output:
    results/mpc_results.csv
    results/mpc_summary.csv
"""

import csv
import os
import numpy as np
from run_experiment_mpc import run_mpc
...
for scenario in SCENARIOS:
    ...
    print(f"\n{'='*60}")
    print(f"  MPC (SOFT)  Scenario: {label} (path_id={path_id})")
    print(f"{'='*60}")
...
# Raw CSV
raw_path = "results/mpc_results.csv"
...
summary_rows = []
print(f"\n{'='*60}")
print("  MPC (SOFT) SUMMARY (mean ± std across seeds)")
print(f"{'='*60}")
...
sum_path = "results/mpc_summary.csv"
if summary_rows:
