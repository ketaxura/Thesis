"""
batch_runner.py
Runs MPCC across all scenarios and seeds, saves CSV, prints summary table.
Usage: python batch_runner.py
Output:
    results/mpcc_results.csv
    results/mpcc_summary.csv
"""

import csv
import os
import numpy as np
from run_experiment import run_mpcc
...
# Summary metrics to print/save (subset for readability)
SUMMARY_METRICS = [
    "goal_reached",
    "completion_time_s",
    "path_efficiency",
    ...
    "max_solve_ms",
    "solver_fail_count",
    "total_body_collisions",
    "dyn_exclusion_violations",
    "near_miss_count",
    "danger_zone_steps",
    "min_clearance_m",
]
...
for scenario in SCENARIOS:
    ...
    print(f"\n{'='*60}")
    print(f"  Scenario: {label} (path_id={path_id})")
    print(f"{'='*60}")

    for seed in range(N_SEEDS):
        print(f"  seed {seed:2d} / {N_SEEDS-1} ...", end=" ", flush=True)
        try:
            result = run_mpcc(path_id=path_id, seed_offset=seed, max_steps=MAX_STEPS)
...
# Raw CSV
raw_path = "results/mpcc_results.csv"
...
sum_path = "results/mpcc_summary.csv"
if summary_rows:
