...
def build_mpcc_solver(ref_traj: np.ndarray, static_rects: list, dyn_obs: list, config: MPCCConfig | None = None) -> MPCCArtifacts:
    if config is None:
        config = MPCCConfig()
    ...
    # S_dyn removed — dynamic obstacles now use hard constraints
    ...
    OPT_vars = ca.vertcat(
        ...
        # S_dyn removed — hard constraints have no slack variable
    )
    ...
    nU = nu * N
    nProg = N + 1
    nSlack_static = num_static_obs * N
    nSlack_dyn = 0   # hard constraints — no slack variables
