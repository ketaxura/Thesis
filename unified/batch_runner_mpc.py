...
def run_batch(use_hard: bool):
    ...
    csv_results = f"results/unified_mpc_{label_suffix.lower()}_results.csv"
    csv_summary = f"results/unified_mpc_{label_suffix.lower()}_summary.csv"
    ...
    for scenario in SCENARIOS:
        ...
        print(f"\n{'=' * 60}")
        print(f"  UNIFIED MPC ({label_suffix})  {label} (path_id={path_id})")
        print(f"{'=' * 60}")
    ...
    summary_rows = []
    print(f"\n{'=' * 60}")
    print(f"  UNIFIED MPC ({label_suffix}) SUMMARY")
    print(f"{'=' * 60}")

    for scenario in SCENARIOS:
        label = scenario["label"]
        runs = [r for r in all_results if r.get("scenario") == label and "ERROR" not in r]
        if not runs:
            continue

        row = {"scenario": label, "controller": f"mpc_{label_suffix.lower()}", "n_runs": len(runs)}
    ...
    if summary_rows:
        with open(csv_summary, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=list(summary_rows[0].keys()), extrasaction="ignore")
            writer.writeheader()
...
if __name__ == "__main__":
    print("Running unified MPC (SOFT) ...")
    run_batch(use_hard=False)

    print("\n\nRunning unified MPC (HARD) ...")
